...
struct DayNightMap: View {
    ...
    // Deep-night tint, matching Android's NIGHT_RGB = 0x0B1020.
    private static let nightTint = Color(red: 0x0B / 255.0, green: 0x10 / 255.0, blue: 0x20 / 255.0)
    // Ocean fallback when no texture is available (Android Color(0xFF0B2A52)).
    private static let oceanFallback = Color(red: 0x0B / 255.0, green: 0x2A / 255.0, blue: 0x52 / 255.0)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .accessibilityLabel(
                "World day and night map. Sun is overhead near latitude "
    ...
    private func overlayCanvas(size: CGSize) -> some View {
        Canvas { ctx, canvasSize in
            ...
            // Zone pins, tinted by whether it is day or night there at `instant`.
            for id in zoneIds {
                let coord = ZoneGeo.coordinate(for: id, at: instant)
